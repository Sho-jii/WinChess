...
namespace Chess
{
    public class Movement {     
        private SoundPlayer captureSound = new SoundPlayer("capture-self.wav");
        private SoundPlayer moveSound = new SoundPlayer("move-self.wav");         
        private SoundPlayer kotoSound = new SoundPlayer("koto.wav");
        Tile pieceTile;
        ...
        public bool isAvailableMove(Tile clickedTile){
            ...
            for (int move=0;move<availableMoves.Length; move++){               
                ...
                if (availableMoves[move]?.x == clickedTile.location.x && availableMoves[move]?.y == clickedTile.location.y){
                    ...
                    if (clickedTile.isOccupied() && clickedTile.piece.color != pieceTile.piece.color)
                    {                   
                        captureSound.Play();
                    }
                    else
